...
public class WorkspaceEventProcessorImpl implements WorkspaceEventProcessor {
    ...
    public Mono<Void> processEvent(Event<?> event, Long workspaceId) {
        ...
        return eventAuditRepository.findById(eventId.getValue())
                ...
                .switchIfEmpty(Mono.defer(() -> {
                    EventAudit started = EventAudit.createStarted(eventId, eventType, "workspace-events");
                ...
                .flatMap(audit -> {
                    ...
                    // 실시간 스트림에 발행
                    return eventBroker.publishEvent(workspaceId, wsPayload)
                            .then(Mono.defer(() -> {
                                audit.markSuccess();
